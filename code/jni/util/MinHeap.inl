...
namespace mage
{
	...
	void FixedSizeMinHeap< capacity, key_t, value_t >::insert( const Key& key, Value& value )
	{
		...
		assertion( m_size < ( CAPACITY - 1 ), "MinHeap buffer overflowed!" );
		...
		assertion( isValidHeap( getFirstNode() ), "MinHeap failed to heapify after inserting element!" );
	}
	...
	void FixedSizeMinHeap< capacity, key_t, value_t >::update( const Key& key, Value& value )
	{
		...
		Node nodeFound = findNodeByValue( value );

		if( nodeFound != nullptr )
		{
			// If the node was found, change the value of the node in the tree.
			nodeFound->key = key;
	...
	template< size_t capacity, typename key_t, typename value_t >
	bool FixedSizeMinHeap< capacity, key_t, value_t >::hasValue( const value_t& value ) const
	{
	...
	template< size_t capacity, typename key_t, typename value_t >
	typename FixedSizeMinHeap< capacity, key_t, value_t >::Node FixedSizeMinHeap< capacity, key_t, value_t >::findNodeByValue( const value_t& value )
	{
	...
	template< size_t capacity, typename key_t, typename value_t >
	typename FixedSizeMinHeap< capacity, key_t, value_t >::ConstNode FixedSizeMinHeap< capacity, key_t, value_t >::findNodeByValue( const value_t& value ) const
	{
		ConstNode nodeFound = nullptr;

		// Find the element in the array.
		for( size_t i = 0; i < m_size; ++i )
		{
			ConstNode currentNode = getNode( i );
	...
	void FixedSizeMinHeap< capacity, key_t, value_t >::bubbleUp( Node node )
	{
		...
		while( getIndexOfNode( node ) > 0 )
		{
			...
			assertion( parentNode < node, "MinHeap parent node is greater than or equal to its child!" );
	...
	void FixedSizeMinHeap< capacity, key_t, value_t >::bubbleDown( Node node )
	{
		...
		while( isValidNode( node ) )
		{
			...
			assertion( firstChildNode > node, "MinHeap first child of parent node is less than or equal to its parent!" );
			assertion( secondChildNode > node, "MinHeap second child of parent node is less than or equal to its parent!" );
