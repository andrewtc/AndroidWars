...
namespace mage
{
	...
	public:
		...
		{
			for ( HashMap< DictionaryValueBase* >::const_iterator itr = other.mDictionary.begin();
		...
		template< typename T >
		DictionaryError Get( const HashString& key, T& value ) const
		{
			// Error on empty
			...
			HashMap< DictionaryValueBase* >::const_iterator found = mDictionary.find( key );
		...
		// Get a named property from the Dictionary of a given type
		// Returns a const ref to the value or defaultValue if not found
		template< typename T >
		const T& Get( const HashString& key, const T& defaultValue=T() ) const
		{
			// Error on empty
			if ( mDictionary.empty() )
			{
	...
	private:
		HashMap< DictionaryValueBase* > mDictionary;
	};
	//---------------------------------------
